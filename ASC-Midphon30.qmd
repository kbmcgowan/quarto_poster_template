...
-   Social perception is more sensitive to SC than vowel quality (@fig-model-predictions). Centers may be more useful to listeners as a source of social information, but they are not essential.

-   Researchers who use a single point measure to characterize the vowels of a talker or of a community are missing a great deal of information that listeners use in perception.
...
-   Sound change: these results are inconsistent with the use of simplified vowel representations and support a coarticulatory path to sound change [@beddor2009]

-   If normalization happens at all, these results are inconsistent with a model of normalization that operates on a single F1/F2 measure (with or without duration) and more consistent with, e.g., @fruehwald2025vowel
