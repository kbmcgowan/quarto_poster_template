...
traj_idct_unnorm |>
  ...
  labs(
   ...
   theme(
    text = element_text(family = "Doulos SIL"),
...
::: {.block fill="rgb(38, 92, 164)" inset="25pt" radius="15pt" stroke="0pt + white"}

[Summary: While Southern talkers were perceived less accurately overall, there is no significant difference between listeners' ability to perceive either vowel quality or region in the Full Vowel and Silent Centers vowel manipulation conditions.]{style="color: white;"}
...
#| warning: false
#| fig-align: center
#| fig-cap: "Model coefficient parameter estimates for 'what?' and 'who?' trials"
#| out-width: 90%

what_param <- what_mod1 |>
  gather_draws(
    `b_.*`,
    regex = T
  ) |>
  ...
  mutate_at(
    ".variable", 
    str_replace, "manipulationSilentcenter", "Manipulation (SC)"
    ) |>
  mutate_at(
    ".variable", 
    str_replace, "regionSouthern", "Region (Southern)"
  ) |>
  mutate_at(
    ".variable", 
    str_replace, "tense_laxtense", "Vowel quality (tense)"
  ) 
...
what_param_dist <- what_param |>
  ...
  labs(
    ...
    title = "'What' trials parameter estimates"
  ) +
  annotate(
    geom = "rect",
    ymin = "Region (Southern):Vowel quality (tense)",
    ymax = "Vowel quality (tense)",
    xmin = -0.18,
    xmax = 0.18,
    fill = "grey",
    alpha = 0.6
...
who_param_dist <- who_param |>
  ...
  labs(
    y = "Predictor",
    x = "Parameter estimate",
    title = "'Who' trials parameter estimates"
  ) +
  annotate(
    geom = "rect",
    ymin = "Manipulation (SC):Region (Southern)",
    ymax = "Manipulation (SC):Region (Southern):Vowel quality (tense)",
    xmin = -0.18,
    xmax = 0.18,
    fill = "grey",
    alpha = 0.6
...
-   Tense/Lax: it may be that listeners need a greater percentage of a more dynamic vowel quality to perceive regional variation or this may be due to varying levels of awareness of particular vowel qualities [@Babel2025]

-   These results are inconsistent with models of sound change or normalization that operate on a single F1/F2 measure (with or without duration) and more consistent with, e.g., @beddor2009 (sound change) or @fruehwald2025vowel (normalization)
